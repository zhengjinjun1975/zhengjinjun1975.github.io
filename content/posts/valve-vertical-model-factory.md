---
title: '模型的终点不是训练完成，而是跟工厂一起长：一个阀门垂直模型怎么做起来'
date: 2026-08-20
draft: false
slug: valve-vertical-model-factory
summary: "一个阀门厂不需要能聊康德的大模型，需要的是回答这台球阀为什么内漏时直接给排查方向的小模型。本文完整复盘一个阀门垂直模型的训练过程：数据集怎么造、基座怎么选、评测怎么避坑、模型怎么跟工厂一起长。"
tags: ["垂直模型", "阀门", "微调", "QLoRA", "单体智能", "中小企业", "数据不出厂"]
categories: ["技术", "AI落地"]
---
...
| 参数 | 取值 |
|------|------|
| 基座模型 | Qwen3.5-4B-Base（无思考） |
| 训练数据 | industrial_base_valve_train_v3 |
| 样本量 | 8620 条（泵阀 63% + 工业 37%） |
| LoRA rank / alpha | r=16 / α=16 |
| LoRA dropout | 0.0 |
| 目标模块 | 全量线性层（q/k/v/o/gate/up/down） |
| 有效 batch size | 1 × 梯度累积 4 = 4 |
| 学习率 | 2e-4（AdamW，cosine 衰减） |
| 训练轮数 | 3 epoch |
| warmup | 约单轮 10%（226 步） |
| weight_decay | 0.01 |
| max_length | 2048 token |
| 保存间隔 | save_steps=200（断电容灾） |
| 训练时长 | 约 12 小时（8GB 单卡） |
| 最终 token 准确率 | 88.4%（最高 89.3%） |
